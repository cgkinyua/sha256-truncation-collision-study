# src/simulate_post_quantum_hashing_gpu.py
...
if __name__ == "__main__":
    ...
    num_keys = 10**7  # Reduce number of keys for faster execution
    num_iterations = 5  # Reduce iterations for efficiency
    batch_size = 10**5  # Process keys in smaller batches of 100,000
    
    results = []
    print(f"Starting optimized GPU-based simulation with varying key lengths...")
    ...
    plt.title("Collision Probability vs. Key Length (Post-Quantum Keys with GPU-Based Hashing)", fontsize=14)
    plt.grid(True)
    plt.xticks(key_lengths)
    plt.savefig("../results/collision_probability_vs_key_length_post_quantum_gpu.png", dpi=300)
    plt.show()
