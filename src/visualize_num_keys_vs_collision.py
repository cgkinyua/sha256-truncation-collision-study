# src/visualize_num_keys_vs_collision.py
...
if __name__ == "__main__":
    num_keys_list = [10**3, 10**4, 10**5, 10**6]  # Vary number of keys
    key_length = 32  # Fixed key length
    num_iterations = 100  # Adjust as needed
    
    results = []
    print(f"Starting visualization with varying number of keys ({key_length}-bit keys)...")
    ...
    plt.plot(num_keys, probabilities, marker='o', linestyle='-', color='green')
    plt.xscale('log')
    plt.xlabel("Number of Keys", fontsize=12)
    plt.ylabel("Estimated Collision Probability", fontsize=12)
    plt.title(f"Collision Probability vs. Number of Keys ({key_length}-bit keys)", fontsize=14)
    plt.grid(True)
    plt.savefig(f"../results/collision_probability_vs_num_keys_{key_length}_bits.png", dpi=300)
    plt.show()
